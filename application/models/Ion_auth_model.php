...
class Ion_auth_model extends CI_Model
{
	...
	public function register($identity, $password, $email, $additional_data = [], $groups = [])
	{
		...
		$data = [
			...
			'username' => $additional_data['first_name'].".".$additional_data['last_name'],
			'password' => $password,
			'email' => $email,
...
//die('adsfa');
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description, '.$this->tables['groups'].'.check_permission ')
	...
	public function get_all_permissions_by_groupids($group_id){
		...
		foreach($group_id as $role_row){
			...
			//$this->db->join('groups g','g.check_permission = on');
			$this->db->where('rp.group_id', $role_row->id);
			//$this->db->where('g.id',$role_row->id);
			$query =  $this->db->get();
